...
def get_velocity():
    ...
    #get the best score of this round and add to mean
...
def PSO_SA_algorithm():
    global epoch
    global mean
    done = False
    
    initialize_particles()
    temp_start = float("inf")
    temp = temp_start
    temp_end = 10
    lowest = float("inf")
    last = float("inf")
    # annealing factor: reduce temp by this factor each try
    coolingFactor  = 0.99
    ...
    while not done:
        ...
        if epoch < MAX_EPOCHS and temp > temp_end:
            ...
            for i in range(PARTICLE_COUNT):
                ...
                #sys.stdout.write("Temperature:"+ str(temp) + "\n")
                newBest = particles[i].get_pBest()
                
                if newBest < lowest: lowest = newBest
                difference = newBest - last
                prob = math.exp(-difference / temp)
                if difference < 0 or prob > random.uniform(0,1): 
                    last = newBest
